...
const settleDuel = async (settlements: DuelSettlement[]) => {
    ...
    const flashDuels = new ethers.Contract(netMap[networkName].FlashDuels, FlashDuelsABI, deployer) as unknown as FlashDuels;
    ...
    for (const settlement of settlements) {
        try {
            ...
            console.log(`Duel ${settlement.duelId} settled with winner ${settlement.winningOptionIndex}. Transaction hash: ${receipt?.hash}`);
            settledDuels.push({
