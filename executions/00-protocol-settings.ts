...
const main = async () => {
    ...
    const flashDuels: FlashDuels = new ethers.Contract(
        netMap[networkName].FlashDuels,
        FlashDuelsABI,
        deployer
    );
    const flashUSDC: FLASHUSDC = new ethers.Contract(
        netMap[networkName].FLASHUSDC,
        FLASHUSDCABI,
        deployer
    );
    ...
    tx = await flashDuels.setPriceAggregators(
        pythSupportedTokens,
        pythSupportedTokensAggrgators
    );
    await tx.wait(1);

    console.log("Set Pyth Supported tokens aggrgator");

    tx = await flashDuels.setProtocolAddress(
        networkConfig[networkName].protocolTreasury
    );
    await tx.wait(1);
