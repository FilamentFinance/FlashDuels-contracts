...
contract FlashDuels is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event CryptoDuelJoined(
        string duelId, string topic, address participant, address token, uint256 amount, uint256 joinTime
    );
        ...
        whenNotPaused
    {
        ...
        emit CryptoDuelJoined(_duelId, duel.topic, msg.sender, _token, _amount, block.timestamp);
    }
    ...
    function cancelDuelIfThresholdNotMet(string calldata _duelId) external onlyBot {
        ...
        emit DuelCancelled(_duelId, duel.startTime, block.timestamp);
    }
    ...
    function getDuelUsersForOption(string memory _duelId, string memory _option) public view returns(address[] memory) {
